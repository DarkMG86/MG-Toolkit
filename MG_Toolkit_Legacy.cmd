...
:OSNoOK
	...
	echo     Ce programme supporte de Windows XP / Server 2003 a Windows 8.1 / Server 2012 R2
...
:main
	...
	if /i "%choix%"=="1" (
		if "%build_win%"=="5" (
			echo.
			echo Cette fonctionnalite n'est pas compatible avec Windows XP / Server 2003 !
			pause
			goto main
		)
		goto activation_mas
	...
	if /i "%choix%"=="4" (
		...
		if %errorlevel%==0 (
			echo.
			echo Cette fonctionnalite n'est pas compatible avec Windows Vista / Server 2008 !
			pause
			goto main
		)
	...
	if exist "%~dp0*.cab" (
		...
		for /f "delims=" %%i in ('dir /B "%~dp0*.cab"') do (
			...
			if exist "%SYSTEMROOT%\System32\dism.exe" (
				...
				pkgmgr /ip /m:"%~dp0%%i" /quiet >nul 2>&1
				set "error=!errorlevel!"
			)
			if !error!==0 (
				echo OK
	...
	if exist "%~dp0*.msu" (
		...
		for /f "delims=" %%i in ('dir /B "%~dp0*.msu"') do (
			...
			if exist "%SYSTEMROOT%\System32\dism.exe" (
				...
				wusa.exe "%~dp0%%i" /quiet /norestart >nul 2>&1
				set "error=!errorlevel!"
			)
			if !error!==0 (
				echo OK
